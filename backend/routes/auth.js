...
router.post('/passkey/register/verify', async (req, res) => {
  try {
    ...
    if (!email || !registrationResponse) {
      return res.status(400).json({ error: 'email and registrationResponse required' });
    }
...
router.post('/passkey/login/options', async (req, res) => {
  try {
    ...
    if (!user || user.passkeys.length === 0) {
      return res.status(404).json({ error: 'No passkeys registered for this email' });
    }
...
router.post('/passkey/login/verify', async (req, res) => {
  try {
    ...
    if (!email || !authenticationResponse) {
      return res.status(400).json({ error: 'email and authenticationResponse required' });
    }

    const user = await User.findOne({ email });
    if (!user || !user.currentChallenge) {
      return res.status(400).json({ error: 'No pending authentication for this email' });
    }
